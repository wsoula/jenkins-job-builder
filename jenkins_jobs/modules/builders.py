...
def xcode(registry, xml_parent, data):
    ...
    XML.SubElement(xcode, 'manualSigning').text = str(has_provisioning_profiles or False).lower()
    if has_provisioning_profiles:
        ...
        for provisioning_profile in data.get('provisioning-profiles'):
            provisioning_profile_xml = XML.SubElement(provisioning_profiles_xml, 'au.com.rayh.ProvisioningProfile')
            convert_mapping_to_xml(provisioning_profile_xml, provisioning_profile, mapping, fail_required=True)
